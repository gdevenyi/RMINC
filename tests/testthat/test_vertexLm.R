...
rmincLm <- verboseRun(
  "vertexLm(testFilesLeft ~ Age,gftest)",
  getOption("verbose")
)
rmincLm2 <- verboseRun(
  "vertexLm(testFilesLeft ~ Age,gftest2,column='thk')",
  getOption("verbose")
)
...
test_that("vertexLm Two Factors for .csv.gz file", {
  ...
  expect_that(
    attr(rmincLm, "df")[[2]],
    is_equivalent_to(attr(rmincLm2, "df")[[2]])
  )
})
...
writeVertex(
  rmincLm,
  file.path(dataPath, "test_lm_with_header.txt"),
  col.names = F,
  header = T
)
writeVertex(

...
simplify_masked <- function(result_list) {
  ...
  lgl_missing <- vapply(
    result_list,
    function(res) {
      inherits(res, "RMINC_MASKED_VALUE") || all(is.na(res))
    },
    logical(1)
  )
  ...
  if (!is.null(result_attributes)) {
    simplified_results <- setMincAttributes(
      simplified_results,
      result_attributes
    )
  }
...
likeVolume <- function(x, strict = TRUE) {
  ...
  if (is.null(volFile) && strict) {
    stop(deparse(substitute(x)), " does not have an associate likeVolume")
  }
...
maskFile <- function(x, strict = TRUE) {
  ...
  if (is.null(maskFile) && strict) {
    stop(deparse(substitute(x)), " does not have an associated mask file")
  }
...
mincFileCheck <- function(filenames) {
  ...
  for (i in 1:length(filenames)) {
    if (
      is.na(filenames[i]) || file.access(as.character(filenames[i]), 4) == -1
    ) {
      stop(
        "The following file could not be read (full filename is between the dashes): ---",
        filenames[i],
        "---"
      )
    }
...
mincGetWorldVoxel <- function(filenames, v1, v2 = NULL, v3 = NULL) {
  ...
  attr(output, "voxelCoord") <- mincConvertWorldToVoxel(
    filenames[1],
    v1,
    v2,
    v3
...
#' @export
mincConvertWorldToVoxel <- function(
  filename,
  v1,
  v2,
  v3,
...
#' @export
mincWriteVolume.mincSingleDim <- function(
  buffer,
  output.filename,
  clobber = NULL,
  ...
) {
  mincWriteVolume.default(
    buffer,
    output.filename,
    attr(buffer, "likeVolume"),
    clobber
  )
}
...
#' @export
mincWriteVolume.mincMultiDim <- function(
  buffer,
  output.filename,
  column = 1,
  like.filename = NULL,
...
#' @export
mincWriteVolume.default <- function(
  buffer,
  output.filename,
  like.filename,
  clobber = NULL,
  ...
  if (file.exists(output.filename) && is.null(clobber)) {
    ...
    if (substr(answer, 1, 1) == "n") {
      stop(
        "Output file exists, specify clobber, or change the output file name."
      )
    }
  ...
  if (
    length(which(is.nan(buffer))) != 0 ||
      length(which(is.infinite(buffer))) != 0 ||
      length(which(is.na(buffer))) != 0
  ) {
    stop(
      "Cannot write volumes with inf,na or nans in them. Please remove the offending character"
    )
  }
  ...
  history <-
    sprintf(
      "%s>>> RMINC output volume, from a(n) %s buffer, like %s",
...
minc.get.history <-
  function(filename) {
    ...
    history <-
      .Call("get_minc_history", as.character(filename))
...
#' @export
minc.append.history <-
  function(filename, new_history = NULL) {
    stopifnot(!is.null(filename))
...
mincTable <- function(filenames, mask = NULL, file_backed = FALSE, ...) {
  ...
  if (any(not_equals_first)) {
    stop(
      "Some minc files supplied to mincTable are the wrong size:\n",
  ...
  if (!is.null(mask)) {
    ...
    if (length(mask_vol) != total_vox) {
      stop(
        "Your mask is not the same size as your input files, ensure it has",
...
extract_column <- function(filename, column = 1) {
  ...
  if (ext %in% c('gz', 'xz', 'bz2', 'GZ', 'XZ', 'BZ2')) {
    # it's compressed file, but we don't care since it can be read
...
writeVertex <- function(
  ...
  if (headers == TRUE || col.names == TRUE) {
    ...
    colnames(vertexData) <- gsub('[\\(\\)]', '', colnames(vertexData), perl = T)
  }
  ...
  if (headers == TRUE) {
    ...
    if (is.data.frame(gf)) {
      ...
      write.table(gf, file = filename, append = TRUE, row.names = FALSE)
      write("</matrix>", file = filename, append = TRUE)
    }
...
parseLmFormula <- function(formula, data, mf) {
  ...
  return(list(
    data.matrix.left = data.matrix.left,
...
#' @export
mincSelectRandomVoxels <- function(
  volumeFileName,
  nvoxels = 50,
  convert = TRUE,
  ...
...
tableOpenFiles <- function() {
  if (
    system("command -v lsof", ignore.stderr = TRUE, ignore.stdout = TRUE) != 0
  ) {
...
enoughAvailableFileDescriptors <-
  function(n, error = TRUE) {
    ...
    if (error & !enough_avail) {
      stop(
        "Not enough available file descriptors to open ",
        n,
        " files, maybe try setting ulimit -Sn <some-larger-number>"
      )
    }
...
## NA corrector
setNA <- function(x, val) {
  x[is.na(x)] <- val
  x
}

## NaN corrector
setNaN <- function(x, val) {
  x[is.nan(x)] <- val
  x
}
...
runRMINCTestbed <- function(
  ...
  cat(
    "\n\nRunning tests in: ",
    paste(rmincPath, "/", "user_tests/", sep = ""),
    "\n\n\n"
  )
  testReport <- testthat::test_dir(
    paste(rmincPath, "/", "user_tests/", sep = ""),
    env = tenv,
    ...
  )
...
getRMINCTestData <- function(
  ...
  if (!file.exists(extractedPath)) {
    ...
    file.rename(
      file.path(dataPath, "RMINC-test-data-main/rminctestdata/"),
      file.path(dataPath, "rminctestdata/")
    )
    ...
    filesToFix <-
      c("filenames.csv", "minc_summary_test_data.csv", "test_data_set.csv") %>%
